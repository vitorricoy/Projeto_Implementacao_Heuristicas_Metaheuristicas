...
std::vector< std::vector<int> > readDistances(std::ifstream& inputFile, int nbClients) {
    std::vector< std::vector<int> > distances(nbClients + 1, std::vector <int>(nbClients + 1));
...
Params::Params(std::string pathToInstance, int nbVeh, int seedRNG) : nbVehicles(nbVeh)
{
	...
	if (inputFile.is_open())
	{
		...
		for (inputFile >> content ; content != "NODE_COORD_SECTION" ; inputFile >> content)
		{
			...
			else if (content == "EDGE_WEIGHT_SECTION")	distances = readDistances(inputFile, nbClients);
			else if (content == "NODE_COORD_TYPE") inputFile >> content2 >> content3;
			else if (content == "NUMBER_VEHICLES") inputFile >> content2 >> nbVehicles;
